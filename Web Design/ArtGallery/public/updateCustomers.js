...
function updateButtonClick() {
  ...
  var payloadCustomerID = row.firstElementChild.textContent; //This is the SQL ID number
  ...
  req.open("GET", "http://flip1.engr.oregonstate.edu:" + port + "/safeUpdateCustomers?customerID=" + payloadCustomerID + "&customerFirstName=" + payloadCustomerFirstName + "&customerLastName=" + payloadCustomerLastName, true);
  ...
  req.addEventListener('load', function () {
    ...
    if (req.status >= 200 && req.status < 400) { //if request send is good do this:
      ...
      window.location.replace("http://flip1.engr.oregonstate.edu:" + port + "/customers");
