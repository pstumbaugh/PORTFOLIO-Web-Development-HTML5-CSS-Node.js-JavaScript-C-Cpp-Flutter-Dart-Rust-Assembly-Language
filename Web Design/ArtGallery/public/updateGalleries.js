...
function updateButtonClick() {
  ...
  var payloadGalleryID = row.firstElementChild.textContent; //This is the SQL ID number
  var payloadGalleryName = document.getElementById("galleryNameForm").value;

  //send an insert request to our server via GET
  req.open("GET", "http://flip1.engr.oregonstate.edu:" + port + "/safeUpdateGalleries?galleryID=" + payloadGalleryID + "&galleryName=" + payloadGalleryName, true);
  ...
  req.addEventListener('load', function () {
    ...
    if (req.status >= 200 && req.status < 400) { //if request send is good do this:
      ...
      window.location.replace("http://flip1.engr.oregonstate.edu:" + port + "/galleries");
