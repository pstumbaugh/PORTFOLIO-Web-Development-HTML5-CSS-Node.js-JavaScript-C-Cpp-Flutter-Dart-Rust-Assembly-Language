...
//opens preludes file, saves it to calling object of preludes class
//returns false if there are any errors
bool Preludes::initialize()
{
    ifstream file;
    bool flag = true;
...
//Attempt to open data file
    file.open("preludes.txt"); 
    if(file.good() != true)  // If file open failed...
    {
        cout << "ERROR: UNABLE TO OPEN PRELUDES FILE" << endl;
        flag = false;
    }
    ...
    while(!file.eof())
    {
        ...
        {
            cout << "ERROR IMPORTING PRELUDES FROM FILE" << endl;
            flag = false;
            break;
        };
...
//saves all preludes to preludes file
void Preludes::saveToFile() const
{
    ofstream out;
    const char *fileName = "preludes.txt";
...
bool Preludes::addNewSongFromFile(char *newSongTitle, char *newComposer, 
...
void Preludes::displaySongTitles()
{
    Node *current = head;
    while (current != nullptr)
    {
...
void Preludes::displaySongTitlesAndDateMOD()
{
    Node *current = head;
    
    if (current == nullptr) // empty list 
    {
        cout << "No Preludes saved." << endl << endl;
        return;
    }
    while (current != nullptr)
    {
...
void Preludes::dispalyAllInfoSpecificSong()
{
    int songNumber = 1;
...
void Preludes::displayAllSongInfo()
{
    Node *current = head;
    while (current != nullptr)
    {
...
void Preludes::Node::displaySingleSongTitle()
{
    cout << songTitle;
}
...
bool Preludes::Node::editSongTitle(char *newSongTitle)
{
...
bool Preludes::Node::editComposer(char *newComposer)
{
...
bool Preludes::Node::editYearComposed(int newYearComposed)
{
...
bool Preludes::Node::editBookTitle (char *newBookTitle)
{
...
bool Preludes::Node::editPageNumber (char *newPageNumber)
{
...
bool Preludes::Node::editPhysicalDescription (char *newPhysicalDescription)
{
...
bool Preludes::Node::editDateMOD(char *newDateMOD)
{
...
void Preludes::Node::displaySingleSongAllInfo(Node *current)
{
    cout << current->songTitle << endl;
...
std::ostream& operator<<(std::ostream& out, Preludes& songList)
{
    Preludes::Node *temp = songList.head;
    while (temp != nullptr)
    {
