...
class RDTLayer(object):
    ...
    def processReceiveAndSendRespond(self):
        ...
        if incomingSize != 0:
            ...
            while counter < incomingSize and self.windowSize <= self.FLOW_CONTROL_WIN_SIZE:
                ...
                if listIncomingSegments[counter].payload != '':
                    ...
                    if checkPacketsReceived(listIncomingSegments[counter].seqnum, self.receivedPackets) == False:
                        ...
                        sendingCheckSum = str(segmentAck.calc_checksum(
                            listIncomingSegments[counter].payload[:4]))
                            ...
                            if self.acknum != currentSeqNum:
                                ...
                                if newDataSize <= currentSeqNum:
                                    ...
                                    self.receivedPackets.append(
                                        listIncomingSegments[counter].seqnum)
                                    self.acknum = 4 + \
                                        getAckNum(self.receivedPackets)
                                else:  # new item is out of order
                                    temp = self.newData[:currentSeqNum] + \
                                        currentPayload + \
